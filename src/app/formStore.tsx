	...
	return (
		<MySafeAreaView>
			<Stack paddingHorizontal={"$5"} space={"$4"} jc={"center"} f={1}>
				...
				<Fieldset>
					<Label htmlFor="type">เลือกวัสดุ</Label>
					<Select id="type" value={trash} onValueChange={setTrash}>
						...
						<Select.Content zIndex={200000}>
							...
							<Select.Viewport minWidth={200}>
								<Select.Group>
									<Select.Label>Fruits</Select.Label>
									{TypeTrash === "หมวดกระดาษ"
										? trashItem[0].sub.map((item, i) => {
												return (
													<Select.Item
														index={i}
														key={item}
										...
										: TypeTrash === "หมวดพลาสติก"
										? trashItem[1].sub.map((item, i) => {
												return (
													<Select.Item
														index={i}
														key={item}
